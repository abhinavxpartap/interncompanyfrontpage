...
const List: React.FC = () => {
    ...
    const get = async (page:number) => {
        setIsLoading(true);
        const response = await fetch(`/api/Event/GET/events?page=${page}`);
    ...
    const remove = async ( page:number, id: string) => {
        setIsLoading(true);
        const response = await fetch(`/api/Event/DELETE/event?id=${id}`, {
            method: 'DELETE',
        });
        await response.json();
        get(page);
    ...
    return <PrivateLayout title="Alumel - Events List">
